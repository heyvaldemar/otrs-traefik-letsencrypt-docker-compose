#!/usr/bin/env bash
# otrs-restore-database.sh [backup-file-name]
#
# Replaces the OTRS database with one of the dumps the backups service wrote.
#
#   ./otrs-restore-database.sh               list and ask
#   ./otrs-restore-database.sh <file-name>   restore that one
#
...
DB_NAME="$(env_of OTRS_DB_NAME)"; DB_USER="$(env_of OTRS_DB_USER)"; DB_PASS="$(env_of OTRS_DB_PASSWORD)"
