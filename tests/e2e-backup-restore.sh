#!/bin/bash
# End-to-end tests for the otrs-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-otrs}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-otrs-traefik-letsencrypt-docker-compose.yml}"
...
DB_USER="${OTRS_DB_USER}"
DB_PASS="${OTRS_DB_PASSWORD}"
DB_NAME="${OTRS_DB_NAME}"
DB_HOST="mariadb"
...
test_restore_roundtrip() {
  ...
  echo "  restoring the baseline with ./otrs-restore-database.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./otrs-restore-database.sh "$(basename "$baseline")" \
...
test_data_restore_roundtrip() {
  ...
  echo "  restoring $(basename "$newest") with ./otrs-restore-application-data.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./otrs-restore-application-data.sh "$(basename "$newest")" \
    || { fail "the shipped restore script failed"; return 1; }
