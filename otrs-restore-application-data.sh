...
BACKUP_PATH="/srv/otrs-application-data/backups/"
RESTORE_PATH="/opt/otrs/"
BACKUP_PREFIX="otrs-application-data"

echo "--> All available application data backups:"

for entry in $(docker container exec -it "$OTRS_BACKUPS_CONTAINER" sh -c "ls $BACKUP_PATH")
do
  echo "$entry"
done
...
docker exec -it "$OTRS_BACKUPS_CONTAINER" sh -c "rm -rf ${RESTORE_PATH}* && tar -zxpf ${BACKUP_PATH}${SELECTED_APPLICATION_BACKUP} -C /"
