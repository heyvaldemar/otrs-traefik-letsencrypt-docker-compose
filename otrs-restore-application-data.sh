#!/usr/bin/env bash
# otrs-restore-application-data.sh [backup-file-name]
#
# Replaces OTRS's application data with one of the archives the backups
# service wrote.
#
#   ./otrs-restore-application-data.sh               list and ask
#   ./otrs-restore-application-data.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than otrs.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-otrs}"
APP_SERVICE="otrs"
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using OTRS." >&2
  exit 1
fi
